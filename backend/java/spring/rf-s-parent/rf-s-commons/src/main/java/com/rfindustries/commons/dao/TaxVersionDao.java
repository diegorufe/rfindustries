...
import com.rfindustries.commons.rowmapper.TaxVersionRowMapper;
...
public interface TaxVersionDao extends BaseJDBCCrudRepository<TaxVersionEntity> {
    ...
    @Query(
            ...
            rowMapperClass = TaxVersionRowMapper.class)
    TaxVersionDTO findTaxVersionByTaxIdAndDate(@Param(EntityDefinition.TAX_ID) Long taxId, @Param(EntityDefinition.START_DATE) LocalDate startDate);
    ...
    @Query(
            value = "SELECT tv.*, tx.ID TX_ID, tx.CODE TX_CODE, tx.NAME TX_NAME FROM RF_COMMONS_TAXES_VERSIONS tv " +
                    ...
                    "tv.TAX_ID IN(:TAX_ID) ",
            rowMapperClass = TaxVersionRowMapper.class)
    List<TaxVersionDTO> findTaxVersionsByIds(@Param(EntityDefinition.TAX_ID) Set<Long> taxIds);
}
